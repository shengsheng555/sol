...
async function get_kyber_feed(feed){
    ...
    const expectedRate = new BigNumber(data.expectedRate);
    const slippageRate = new BigNumber(data.slippageRate);
...
async function get_oneinch_feed(feed){
    ...
    feed.expected_return = new BigNumber(data.returnAmount);
...
function get_arbs_cand(){
    ...
    for (var i = 0; i < assetAs.length; i++) {
        ...
        for (var j = 0; j < amount_factors.length; j++) {
            ...
            //     const in_amount = new BigNumber(1).shiftedBy(6);
